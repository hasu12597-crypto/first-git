...
def build_dashboard_html(metrics: Dict[str, Any]) -> str:
    script_data = json.dumps(metrics, ensure_ascii=False).replace('<', '\\u003c')
...
<html lang="ko">
  <head>
    ...
    .level-badge { display: inline-block; margin-top: 10px; padding: 6px 14px; border-radius: 999px; font-size: 14px; font-weight: 700; }
    .level-badge.elite { color: #6422c9; background: #eee7ff; }
    .level-badge.high { color: #3730a3; background: #e0e7ff; }
    .level-badge.medium { color: #1e40af; background: #dbeafe; }
    .level-badge.low, .level-badge.unclassified, .level-badge.unavailable { color: #4b5563; background: #f3f4f6; }
