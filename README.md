...
대시보드는 반올림 전 **평균 Lead Time**으로 배지와 기준표의 현재 구간을 표시합니다. 수업 자료 기준으로 Elite는 1시간 미만, High는 1일 이상~7일 미만, Medium은 7일 이상~30일 미만, Low는 30일 이상입니다. 1개월은 30일로 환산하며, 자료에 빠진 1시간 이상~1일 미만은 **기준 미정**으로 표시합니다. 데이터가 없거나 유효하지 않거나 API 수집 오류가 있으면 **평가 불가**로 표시합니다. 예를 들어 평균 33.3초는 Elite입니다.
